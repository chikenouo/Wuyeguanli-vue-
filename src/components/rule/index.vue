...
import { ruleSearchAll } from '@/api/api'
...
const searchAll = async () => {
  try {
    const result = await ruleSearchAll()
    if (result.statusCode === 200) {
      originalData.value = result.ruleList // 保存原始數據
      filteredData.value = [...originalData.value] // 初始化篩選後的數據
      total.value = filteredData.value.length
      console.log('Search:', originalData.value)
    } else {
      ElMessage.error('獲取規約失敗')
    }
  } catch (error) {
    console.error('獲取規約錯誤:', error)
    ElMessage.error('獲取規約時發生錯誤')
  }
}
...
<template>
  <div class="announcement-page">
    ...
    <!-- 詳情對話框 -->
    <el-dialog v-model="dialogVisible" title="規約詳情" width="60%">
